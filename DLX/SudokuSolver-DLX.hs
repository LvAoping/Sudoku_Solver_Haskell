...
{- 
    This implementation of the Sudoku solver is based on the Dancing Links (DLX) algorithm. 
    - The DLX is a technique for solving exact cover problems, its key idea is the toroidal doubly linked list.
    - It enbales constant-time deletion and restoration of rows and columns in the matrix, thereby optimizes the efficiency of the backtracking algorithm.

    - Exact Cover Problem:
        - The goal of the exact cover problem is to find a subset of rows in a given matrix of 0s and 1s 
        such that each column contains exactly one "1" from the selected rows. In other words, the chosen 
        subset of rows must cover each column without any overlap.

    - Structure of the Dancing links:
        Dancing Links is a specialized doubly linked list structure designed to represent sparse binary matrices. 
        In Dancing Links:
        - Each node has pointers to its left, right, up, and down neighbors.
        - Each "1" in the matrix is represented as a node, with nodes linked together by rows and columns.
        - Each column has a header node that records the count of nodes in that column (i.e., the number of "1"s), which helps in selecting the optimal column.
        In this structure, deleting a row or column of nodes can be done in constant time 
        by simply updating pointers, without the need to shift the entire data structure, 
        thus speeding up the backtracking process for "select-delete-restore" operations.
         
    The sudoku problem can be represented as an exact cover problem,
    - Each cell in the Sudoku board corresponds to a row in the exact cover matrix.
        - Total number of cells in the board: n^2
        - Each cell has n possible values (1 to n).
        - Total number of rows in the matrix: n^3
    - Each row has 4 constraints: cell, block, row, and column constraints, each with n^2 columns.
        - Total number of columns in the matrix: n^2 * 4
    - The DLX matrix is built from the Sudoku board,
        - The dancing links matrix size: n^3 rows and n^2 * 4 columns.

    The DLX works as follows:
    1. Select Column: Choose the column with the fewest nodes (i.e., the fewest "1"s) to minimize branching.
    2. Cover Column: Remove the chosen column and all rows linked to it from the matrix.
    3. Select Row and Recurse: Choose a row in the selected column and recursively search for a solution.
    4. Recursive Search: Repeat the search process until all rows are covered or multiple solutions are found.
    5. Uncover (Backtrack): If no solution is found, backtrack by restoring the column and row to the matrix.

    Based on the DLX algorithm, it is further optimized according to the sudoku's characteristics:
    - Less rows are generated by only considering the cells with multiple candidates.
    - The naked and hidden subsets strategies are applied to refine the candidates before solving the exact cover problem.
    - As the dancing links matrix is shrinked, the search process is speeded up.
 -}
...
{- Acknowledgement:
  - The algorithm implementation of the Sudoku solver is inspired by the following resources:
    ...
    - https://wiki.haskell.org/File:DancingSudoku.lhs
    - https://wiki.haskell.org/Sudoku
    - https://github.com/Determinant/h99/blob/master/95-99/sudoku.c
...
nickname :: String
nickname = "SudokuOuSama-2"



{- DNode s
   Represents a node in the Dancing Links (DLX) data structure, used for exact cover problems like Sudoku.
   Each node contains references to its neighboring nodes in four directions (left, right, up, down), 
   as well as a control node to track the column header.

   nodeSize tracks the count of nodes in the column for column header nodes only.

   rowIdx and colIdx represent the row and column indices of the node.
...
{- getAttr dir node
   Retrieves the attribute specified by 'dir' from the given node.
   RETURNS: The value of the specified attribute.
-}
getAttr :: (DNode s -> STRef s a) -> DNode s -> ST s a
getAttr dir node = readSTRef (dir node)

{- setAttr dir node value
   Sets the attribute specified by 'dir' for the given node to the provided value.
-}
...
{- setCover controlNode
   ...
   PRE: controlNode is a column header node in a valid DLX structure.
-}
...
{- solve bitmap numCols
   Solves the exact cover problem represented by a bitmap using the DLX algorithm.
   PRE: bitmap represents a matrix with rows indicating candidate solutions and columns representing constraints.
   RETURNS: A list of solutions, where each solution is a list of row indices satisfying the exact cover constraints.

   VARIANT: The number of uncovered columns in the DLX matrix, which decreases as columns are covered.
-}
...
solve bitmap numCols = 
    runST $ do
        ...
        readSTRef solutions
  where
    {-  search rootHeader solutions countRef plan
        Recursive search function for solving the exact cover problem using DLX.
        Attempts to find all solutions for a given DLX matrix starting from rootHeader.
        PRE: 
            - rootHeader is the root node of a valid DLX matrix.
            - solutions is a reference to an accumulator list where found solutions are stored.
            - countRef is a reference tracking the number of solutions found so far.
            - plan is the current list of row indices forming a partial solution.
        RETURNS: Adds completed solutions to the solutions list in countRef as it finds valid sets of row indices.
        VARIANT: Number of uncovered columns in the DLX matrix, which decreases as columns are covered.
    -}
...
{- sudoku prob
   Solves a Sudoku puzzle using a DLX-based approach with initial candidate refinement.
   PRE: prob is a valid Sudoku board (a n x n board with n as a perfect square).
   RETURNS: A tuple of (solution boards, solution type) where:
       solution boards: A list of solved Sudoku boards.
       solution type: An indicator of whether the solution is unique, non-existent, or has multiple solutions.
